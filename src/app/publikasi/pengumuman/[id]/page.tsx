...
export default function PengumumanDetailPage() {
  ...
  useEffect(() => {
    async function loadData() {
      if (!id) return;
      try {
        const result = await supabaseFetch("pengumuman", `id=eq.${id}`);
        if (result && result.length > 0) {
          setData(result[0]);
        }
      } catch (e) {
        console.error("Error fetching pengumuman detail:", e);
      } finally {
        setLoading(false);
      }
    }
  ...
  const fileUrl = data?.file_url ? supabaseGetPublicUrl("pengumuman-files", data.file_url) : null;
    ...
    <>
      ...
      <main className="min-h-screen bg-slate-50 pt-24 pb-12 w-full">
        <div className="max-w-4xl mx-auto px-6">
          <Link href="/publikasi/pengumuman" className="inline-flex items-center gap-1 text-primary hover:text-secondary font-medium mb-6 transition-colors">
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
              {fileUrl && (
                <div className="w-full relative bg-slate-100 border-b border-slate-200">
                  {isPdf ? (
                    <div className="w-full h-[80vh] min-h-[600px]">
                      <iframe src={`${fileUrl}#view=FitH`} className="w-full h-full border-none" title="Dokumen Pengumuman" />
                    </div>
                  ) : (
                    <img src={fileUrl} alt={data.judul} className="w-full h-auto max-h-[60vh] object-contain mx-auto" />
                  )}
                </div>
              )}

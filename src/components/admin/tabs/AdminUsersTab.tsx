...
export function AdminUsersTab() {
  ...
  const handleAddUser = async (e: React.FormEvent) => {
    ...
    try {
      // In a real app we'd hash here or create an API route. 
      // Since this is just for the example, I'll call a dedicated API route to hash the password.
      // Wait, we can't hash in the browser securely without bcryptjs loading in browser which is large.
      // So let's create a new API route `/api/auth/register` to add a user.
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 flex flex-col h-fit sticky top-28">
          ...
          <form onSubmit={handleAddUser} className="space-y-4">
            ...
            <div>
              ...
              <select value={role} onChange={e => setRole(e.target.value)} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-blue-500 bg-white">
                <option value="admin">Admin (Konten)</option>
                <option value="super_admin">Super Admin (Konten + Sistem)</option>
              </select>
            </div>
